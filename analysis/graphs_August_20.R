...
plot_sed_time_2 <- together_3 %>%
        ...
        unique()%>%
        ggplot() +
        geom_line(aes(x = sedrate.mm, y = (median/1000)), size = 1, alpha = 0.75) + geom_path(aes(x = sedrate.mm, y = (median/1000)), size = 1, alpha = 0.75)+
        xlab("Sed rate (mm/yr)") +
        ylab("Age (ka)")+
...
together_3%>%
        filter(!is.na())
together_3%>%
        select(sumdepth_sand_coarse)%>%
        filter(!is.na(sumdepth_sand_coarse))%>%
        unique()
